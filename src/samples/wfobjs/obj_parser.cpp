...
int obj_parse_inline_obj_file(obj_growable_scene_data *growable_data, char const *data, const size_t ndata)
{
...
/*		
	...
	//parser loop
	char * current_line = (char *)malloc(ndata);
	strncpy(current_line,data,ndata + 1);
	current_line[ndata] = 0;
...
int parse_inline_obj_scene(obj_scene_data *data_out, char const *data, const size_t ndata)
{
	obj_growable_scene_data growable_data;

	obj_init_temp_storage(&growable_data);
	if( obj_parse_inline_obj_file(&growable_data, data, ndata) == 0)
		return 0;
