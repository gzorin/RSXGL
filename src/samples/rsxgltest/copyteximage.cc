...
const char * rsxgltest_name = "copyteximage";
...
rsxgltest_draw()
{
  ...
  float tmp = 0.0;
  const float elapsed_time_bin = modff(rsxgltest_elapsed_time / 9.0f,&tmp);
  float rgb[3] = {
    (elapsed_time_bin < (1.0f / 3.0f)) ? 1.0f : 0.0f,
     (elapsed_time_bin >= (1.0f / 3.0f) && elapsed_time_bin < (2.0f / 3.0f)) ? 1.0f : 0.0f,
     (elapsed_time_bin >= (2.0f / 3.0f) && elapsed_time_bin < 1.0f) ? 1.0f : 0.0f
     };
