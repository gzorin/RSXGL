...
rsxgl_context_t::rsxgl_context_t(const struct rsxegl_config_t * config,gcmContextData * gcm_context)
  : active_texture(0), any_samples_passed_query(RSXGL_MAX_QUERY_OBJECTS), conditional_render_query(RSXGL_MAX_QUERY_OBJECTS), ref(0), timestamp_sync(0), next_timestamp(1), last_timestamp(0), cached_timestamp(0)
{
